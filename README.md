...
当前版本可以在本地完整体验大部分核心流程，但仍有少数能力依赖特定本机环境，因此不保证所有系统都能 100% 跑通全部链路。
...
这些功能当前依赖本机环境中的特定能力，例如：

- macOS 原生 OCR / Vision 能力
- Swift 运行环境
- 本地 Python / Node 路径配置

因此：

- 推荐优先在 macOS 环境下运行本项目
- Windows / Linux 环境下，部分文件解析与导出能力可能无法直接复现
- 如果缺少对应本机依赖，相关接口可能报错或无法返回结果

另外，本项目不会内置任何可直接使用的 API Key。

如需本地运行，你需要自行在 `.env.local` 中填写可用的 DeepSeek API Key；所有模型调用产生的额度消耗也将由使用者自己的账号承担。
后续如果项目继续演进，这部分能力会逐步改造为更通用的跨平台实现。
