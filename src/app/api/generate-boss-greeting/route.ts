...
type GenerateBossGreetingRequest = {
  jdJson?: unknown;
  draftJson?: unknown;
  jobTitle?: string;
...
export async function POST(request: Request) {
  ...
  let body: GenerateBossGreetingRequest;

  try {
    body = (await request.json()) as GenerateBossGreetingRequest;
  } catch {
  ...
  const model = process.env.DEEPSEEK_BOSS_GREETING_MODEL ?? "deepseek-v4-flash";
  ...
  if (!result) {
    return NextResponse.json({ error: "DeepSeek 没有返回可用的 Boss 打招呼语" }, { status: 502 });
  }
